...
export async function GET(request: Request) {
  ...
  const [profilesResult, subscriptionsResult, ordersResult, productsResult] = await Promise.all([
    ids.length
      ? adminSupabase
          .from("profiles")
          .select(
            "id,full_name,role,system_role,status,created_at,last_seen_at",
          )
          .in("id", ids)
  ...
  const subscriptionMap = new Map(
    (subscriptionsResult.data ?? []).map((subscription: any) => [
...
export async function POST(request: Request) {
  ...
  const end = new Date(
    now.getTime() +
      Math.max(1, Number(access_days) || 30) * 86400000,
  );
  ...
  await adminSupabase.from("subscriptions").upsert(
    {
      ...
      current_period_end:
        system_role === "SUPER_ADMIN" ? null : end.toISOString(),
      access_mode:
        system_role === "SUPER_ADMIN" ? "LIFETIME" : "MANUAL",
...
export async function PATCH(request: Request) {
  ...
  if (
    id === auth.user.id &&
    ["SUSPEND", "DELETE", "SUBSCRIPTION_SUSPEND"].includes(action)
  ) {
    return NextResponse.json(
  ...
  if (action === "SUSPEND") {
    ...
    await adminSupabase
      ...
      .update({
        ...
        access_mode:
          system_role === "SUPER_ADMIN" ? "LIFETIME" : "MANUAL",
        current_period_end:
          system_role === "SUPER_ADMIN" ? null : undefined,
      })
      .eq("user_id", id);
    ...
    const nextEnd = new Date(
      base.getTime() + extensionDays * 86400000,
    );
    ...
    await adminSupabase.from("subscriptions").upsert(
      {
        user_id: id,
        status:
          end.getTime() > Date.now() ? "ACTIVE" : "EXPIRED",
    ...
    await adminSupabase
      ...
      .update({
        status: "SUSPENDED",
        updated_by: auth.user.id,
        notes: "Assinatura suspensa pelo Super Admin",
      })
      .eq("user_id", id);
