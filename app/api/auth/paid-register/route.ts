...
export async function POST(request:Request){
  ...
  const {data:order,error:orderError}=await adminSupabase.from("subscription_orders").select("id,order_nsu,transaction_nsu,plan,status,user_id,claimed_at,amount,coupon_code").eq("order_nsu",claim.order_nsu).maybeSingle();
  ...
  const freeByCoupon=Number(order.amount)===0;
  const {error:subError}=await adminSupabase.from("subscriptions").upsert({user_id:userId,plan:order.plan,status:"ACTIVE",starts_at:now.toISOString(),current_period_start:now.toISOString(),current_period_end:end.toISOString(),access_mode:"PROVIDER",payment_provider:freeByCoupon?"COUPON":"INFINITEPAY",provider_subscription_id:order.transaction_nsu,last_payment_at:now.toISOString(),next_payment_at:end.toISOString(),notes:freeByCoupon?`Primeiro acesso liberado por cupom integral${order.coupon_code?` (${order.coupon_code})`:""}`:"Primeiro acesso liberado após pagamento InfinitePay"},{onConflict:"user_id"}); if(subError){await cleanup();return NextResponse.json({error:subError.message},{status:400})}
