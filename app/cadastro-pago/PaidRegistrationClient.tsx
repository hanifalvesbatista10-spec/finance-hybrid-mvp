...
export function PaidRegistrationClient({token}:{token:string}){
  ...
  const [saving,setSaving]=useState(false),[error,setError]=useState(""),[success,setSuccess]=useState(false),[destination,setDestination]=useState("/dashboard");
  ...
  return <main className="grid min-h-screen place-items-center bg-slate-50 p-4"><Card className="w-full max-w-lg border-0 shadow-2xl"><CardHeader className="items-center text-center">
    ...
    <CardContent>{success?<Link href={`/login?next=${encodeURIComponent(destination)}`}><Button className="h-12 w-full">Entrar no Equity One<ArrowRight className="size-4"/></Button></Link>:
