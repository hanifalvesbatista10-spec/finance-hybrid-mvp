...
export function PaidRegistrationClient({token}:{token:string}){
  ...
  return <main className="grid min-h-screen place-items-center bg-slate-50 p-4"><Card className="w-full max-w-lg border-0 shadow-2xl"><CardHeader className="items-center text-center">
    <Link href="/" className="mb-5 flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-indigo-600 text-white"><Landmark className="size-5"/></span><strong>FINANCE HYBRID</strong></Link>
    ...
    <CardContent>{success?<Link href="/login"><Button className="h-12 w-full">Entrar no Finance Hybrid<ArrowRight className="size-4"/></Button></Link>:
