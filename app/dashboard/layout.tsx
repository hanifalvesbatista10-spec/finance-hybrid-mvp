...
const navFor = (role?:ProfileRole) => role === "INSTITUTIONAL" ? institutional : personal;
...
export default function DashboardLayout({ children }:{ children:React.ReactNode }) {
  ...
  const navigation = useMemo(() => navFor(profile?.role), [profile?.role]);
  ...
  if (loading || !user) return <div className="grid min-h-screen place-items-center bg-slate-50 text-sm text-slate-500">Carregando...</div>;

  return (
    <div className="min-h-screen bg-[#f5f7fb]">
      {open && <button type="button" className="fixed inset-0 z-40 bg-slate-950/45 lg:hidden" onClick={() => setOpen(false)} />}
      <aside className={cn(
        ...
        <div className="flex h-24 items-center justify-between px-6">
          <Link href="/dashboard" className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-xs text-slate-400">{profile?.role === "INSTITUTIONAL" ? "Business" : "Personal"}</span>
            </div>
          </Link>
          <Button variant="ghost" size="icon" className="text-white hover:bg-white/10 lg:hidden" onClick={() => setOpen(false)}><X className="size-5" /></Button>
        </div>

        <div className="mx-4 rounded-2xl border border-white/10 bg-white/[.06] p-4">
          <div className="flex items-center gap-3">
            <span className="grid size-10 place-items-center rounded-xl bg-white/10">
              {profile?.role === "INSTITUTIONAL" ? <Building2 className="size-5" /> : <UserRound className="size-5" />}
            </span>
            <div className="min-w-0">
              <p className="truncate text-sm font-bold">{profile?.full_name || "Usuário"}</p>
        ...
        <nav className="flex-1 space-y-1 px-4 py-7">
          <p className="mb-3 px-3 text-[10px] font-black uppercase tracking-[.2em] text-slate-500">Menu principal</p>
          {navigation.map((item) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active ? "bg-indigo-600 text-white shadow-lg shadow-indigo-950/30" : "text-slate-400 hover:bg-white/[.06] hover:text-white",
                )}
              >
      ...
      <div className="lg:pl-[285px]">
        <header className="sticky top-0 z-30 flex h-16 items-center border-b border-slate-200/80 bg-white/85 px-4 backdrop-blur-xl lg:hidden">
          <Button variant="ghost" size="icon" onClick={() => setOpen(true)}><Menu className="size-5" /></Button>
          <span className="ml-3 font-black text-slate-900">Finance Hybrid</span>
        </header>
        <main className="mx-auto max-w-[1600px] p-4 md:p-8 lg:p-10">{children}</main>
      </div>
    </div>
  );
}
