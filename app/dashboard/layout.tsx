...
import {BarChart3,BellRing,Building2,CalendarClock,CalendarDays,ChevronDown,CreditCard,FileText,Goal,Landmark,LayoutDashboard,LogOut,Menu,ReceiptText,ShieldAlert,ShieldCheck,UserRound,X} from "lucide-react";
...
export default function Layout({children}:{children:React.ReactNode}){
 ...
 const [open,setOpen]=useState(false),[userMenu,setUserMenu]=useState(false);
  ...
  <aside className={cn('fixed inset-y-0 left-0 z-50 flex w-[292px] flex-col bg-[#0b0d11] text-white transition-transform lg:translate-x-0',open?'translate-x-0':'-translate-x-full')}>
   ...
   <div className="mx-4 overflow-hidden rounded-2xl border border-white/[.07] bg-white/[.04]">
    <button type="button" onClick={()=>setUserMenu(v=>!v)} className="flex w-full items-center gap-3 p-4 text-left transition hover:bg-white/[.035]" aria-expanded={userMenu}>
     <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-[#d2aa51]/10 text-[#d2aa51]">{effectiveRole==='INSTITUTIONAL'?<Building2 className="size-5"/>:<UserRound className="size-5"/>}</span>
     <div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{profile?.full_name||'Usuário'}</p><p className="truncate text-xs text-slate-500">{user.email}</p></div>
     <ChevronDown className={cn('size-4 shrink-0 text-slate-500 transition-transform',userMenu&&'rotate-180')}/>
    </button>
    {userMenu&&<div className="border-t border-white/[.06] p-2">
      <Link href="/dashboard/seguranca-reset" onClick={()=>{setUserMenu(false);setOpen(false)}} className={cn('flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition',pathname.startsWith('/dashboard/seguranca-reset')?'bg-rose-500/15 text-rose-200':'text-slate-400 hover:bg-rose-500/10 hover:text-rose-200')}><ShieldAlert className="size-4"/>Segurança e Reset</Link>
      <button type="button" onClick={async()=>{await signOut();router.replace('/login')}} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold text-slate-500 transition hover:bg-rose-500/10 hover:text-rose-300"><LogOut className="size-4"/>Sair</button>
    </div>}
   </div>
