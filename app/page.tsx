...
import {
  ArrowRight, BarChart3, BellRing, BriefcaseBusiness, Building2, CalendarDays,
  Check, ChevronRight, CircleDollarSign, Clock3, FileText, HeartPulse, LineChart,
  LockKeyhole, PieChart, ShieldCheck, Sparkles, Target, TrendingUp, UserRound,
  WalletCards, Zap
} from "lucide-react";
...
const outcomes = [
  ...
  ["Este mês", "Quanto entrou, quanto saiu e o que ainda falta acontecer"],
  ["Próximos dias", "Contas, recebimentos e compromissos antes do vencimento"],
  ["Evolução", "Se suas decisões estão melhorando ou piorando seu resultado"],
];

const products = [
  {
    icon: UserRound, eyebrow:"PARA SUA VIDA FINANCEIRA", title:"Equity One Pessoal", price:"R$ 19,90", href:"/pessoal",
  ...
  {
    icon: Building2, eyebrow:"PARA QUEM ADMINISTRA UMA EMPRESA", title:"Equity One Negócios", price:"R$ 59,90", href:"/negocios",
  ...
  {
    icon: HeartPulse, eyebrow:"EXCLUSIVO PARA MÉDICOS", title:"Equity One Médicos", price:"R$ 59,90", href:"/medicos",
...
export default function HomePage(){
  return <main className="min-h-screen overflow-hidden bg-[#f3f1eb] text-[#0b0c0f]">
    ...
    <section className="border-y border-black/[.06] bg-[#0b0c0f] text-white">
      <div className="mx-auto max-w-7xl px-4 py-20 md:px-8 md:py-24"><div className="max-w-3xl"><p className="text-xs font-black uppercase tracking-[.2em] text-[#d4ad55]">O que muda na prática</p><h2 className="mt-4 text-4xl font-black tracking-[-.035em] md:text-5xl">Você deixa de perguntar “onde foi meu dinheiro?” e começa a perguntar “qual é minha próxima decisão?”.</h2></div><div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{outcomes.map(([a,b])=><div key={a} className="rounded-3xl border border-white/10 bg-white/[.04] p-6"><p className="text-xs font-black uppercase tracking-[.16em] text-[#d4ad55]">{a}</p><p className="mt-4 text-sm font-semibold leading-6 text-slate-300">{b}</p></div>)}</div></div>
    </section>

    <section id="como-funciona" className="mx-auto max-w-7xl px-4 py-20 md:px-8 md:py-28">
      <div className="text-center"><p className="text-xs font-black uppercase tracking-[.2em] text-[#9a762b]">Uma rotina mais simples</p><h2 className="mx-auto mt-4 max-w-3xl text-4xl font-black tracking-[-.035em] md:text-5xl">Registre uma vez. Deixe o sistema transformar o dado em gestão.</h2><p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-slate-600">O Equity One foi pensado para reduzir repetição: a informação cadastrada alimenta indicadores, calendário, alertas e relatórios.</p></div>
      <div className="mt-14 grid gap-5 lg:grid-cols-4">{[
        ["01",Zap,"Registre em segundos","Receita, despesa, conta, plantão ou recebimento entra pelo fluxo certo."],
        ["02",PieChart,"O sistema organiza","A informação passa a compor categorias, períodos, compromissos e indicadores."],
        ["03",BellRing,"Você é avisado","O que vence, atrasa ou exige atenção deixa de depender da memória."],
        ["04",Target,"Você decide melhor","O dashboard mostra contexto para agir antes de comprometer o resultado."],
      ].map(([n,I,t,d]:any)=><div key={n} className="relative rounded-[2rem] border border-black/[.06] bg-white p-7"><span className="text-4xl font-black text-black/[.06]">{n}</span><I className="mt-7 size-6 text-[#9a762b]"/><h3 className="mt-4 text-lg font-black">{t}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{d}</p></div>)}</div>
    </section>

    <section id="solucoes" className="border-y border-black/[.06] bg-white/65">
      <div className="mx-auto max-w-7xl px-4 py-20 md:px-8 md:py-28">
        <div className="max-w-3xl"><p className="text-xs font-black uppercase tracking-[.2em] text-[#9a762b]">Não existe uma gestão única para todo mundo</p><h2 className="mt-4 text-4xl font-black tracking-[-.035em] md:text-5xl">Cada rotina tem perguntas diferentes. O dashboard também precisa ter.</h2><p className="mt-5 text-base leading-7 text-slate-600">Por isso Pessoal, Negócios e Médicos são experiências independentes. Você contrata somente o produto feito para o seu contexto.</p></div>
        <div className="mt-12 grid gap-5 lg:grid-cols-3">{products.map(p=><ProductCard key={p.title} {...p}/>)}</div>
      </div>
    </section>

    <section className="mx-auto max-w-7xl px-4 py-20 md:px-8 md:py-28">
      <div className="grid gap-8 rounded-[2.5rem] border border-black/[.06] bg-white p-7 shadow-[0_30px_80px_rgba(15,23,42,.07)] md:p-10 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
        <div><p className="text-xs font-black uppercase tracking-[.2em] text-[#9a762b]">Planilha registra. Gestão antecipa.</p><h2 className="mt-4 text-4xl font-black tracking-[-.035em]">O Equity One foi construído para tirar informação do arquivo e colocar decisão no dashboard.</h2><p className="mt-5 text-sm leading-7 text-slate-600">Você continua no controle dos seus dados, mas deixa de depender de lembrar onde registrou cada coisa ou de montar a leitura do mês manualmente.</p></div>
        <div className="grid gap-3 sm:grid-cols-2">{[
          [CalendarDays,"Calendário financeiro","Veja o mês como uma linha do tempo financeira."],
          [BellRing,"Alertas úteis","Destaque para o que exige ação, não ruído."],
          [BarChart3,"Indicadores claros","Números que respondem perguntas, não só ocupam cards."],
          [FileText,"Relatórios","Transforme histórico em acompanhamento."],
          [Clock3,"Menos repetição","Fluxos pensados para reaproveitar dados já cadastrados."],
          [CircleDollarSign,"Visão de resultado","Entenda o que realmente sobra depois dos compromissos."],
        ].map(([I,t,d]:any)=><div key={t} className="rounded-2xl bg-[#f5f3ed] p-5"><I className="size-5 text-[#9a762b]"/><h3 className="mt-4 text-sm font-black">{t}</h3><p className="mt-2 text-xs leading-5 text-slate-500">{d}</p></div>)}</div>
      </div>
    </section>

    <section id="seguranca" className="border-y border-black/[.06] bg-[#e9e5da]">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-20 md:px-8 lg:grid-cols-[.75fr_1.25fr] lg:items-center"><div><span className="grid size-14 place-items-center rounded-2xl bg-[#0b0c0f] text-[#d4ad55]"><LockKeyhole className="size-6"/></span><p className="mt-6 text-xs font-black uppercase tracking-[.2em] text-[#8e6924]">Dados financeiros exigem seriedade</p><h2 className="mt-3 text-4xl font-black tracking-[-.035em]">Privacidade e controle fazem parte do produto.</h2></div><div className="grid gap-4 sm:grid-cols-2">{[
        [ShieldCheck,"Acesso individual","Cada conta acessa somente os próprios dados dentro da aplicação."],
        [CircleDollarSign,"Pagamento verificado","A liberação do acesso depende da confirmação do pagamento no servidor."],
        [BriefcaseBusiness,"Produtos isolados","Pessoal, Negócios e Médicos mantêm jornadas e experiências próprias."],
        [Check,"Você continua no comando","O Equity One organiza e apresenta informações; não movimenta seu dinheiro por conta própria."],
      ].map(([I,t,d]:any)=><div key={t} className="rounded-2xl border border-black/[.05] bg-white/70 p-5"><I className="size-5 text-[#8e6924]"/><h3 className="mt-4 font-black">{t}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{d}</p></div>)}</div></div>
    </section>

    <section className="relative overflow-hidden bg-[#0b0c0f] text-white"><div className="absolute left-1/2 top-0 h-64 w-[700px] -translate-x-1/2 rounded-full bg-[#c9a34d]/10 blur-3xl"/><div className="relative mx-auto max-w-5xl px-4 py-24 text-center md:px-8 md:py-28"><p className="text-xs font-black uppercase tracking-[.22em] text-[#d4ad55]">A decisão começa com visibilidade</p><h2 className="mx-auto mt-5 max-w-4xl text-4xl font-black leading-tight tracking-[-.04em] md:text-6xl">Seu dinheiro já está contando uma história. <span className="text-[#dfbf70]">O Equity One ajuda você a enxergá-la a tempo de mudar o próximo capítulo.</span></h2><p className="mx-auto mt-6 max-w-2xl text-base leading-7 text-slate-400">Escolha a experiência correspondente à sua rotina e comece a transformar movimentação em gestão.</p><a href="#solucoes"><Button className="mt-8 h-14 rounded-2xl bg-[#d4ad55] px-8 text-base font-black text-black hover:bg-[#e0c278]">Escolher meu Equity One<ArrowRight className="size-5"/></Button></a></div></section>

    <footer className="bg-[#08090b] text-slate-500"><div className="mx-auto flex max-w-7xl flex-col gap-5 px-4 py-8 text-xs md:flex-row md:items-center md:justify-between md:px-8"><div><strong className="tracking-[.2em] text-white">EQUITY ONE</strong><p className="mt-1">Gestão financeira inteligente.</p></div><div className="flex gap-5"><Link href="/login" className="hover:text-white">Entrar</Link><a href="#solucoes" className="hover:text-white">Soluções</a><a href="#seguranca" className="hover:text-white">Segurança</a></div></div></footer>
  </main>
}
