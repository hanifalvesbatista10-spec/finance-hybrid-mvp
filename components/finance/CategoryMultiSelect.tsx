...
export function CategoryMultiSelect({
  ...
  const builtInItems = useMemo(() => groups.flatMap((group) => group.items), [groups]);
  const allItems = useMemo(() => [...builtInItems, ...customItems], [builtInItems, customItems]);
  ...
  const filteredCustom = useMemo(
    () => customItems.filter((item) => !normalized || item.toLowerCase().includes(normalized)),
            ...
            {filteredCustom.length > 0 && (
              <div className="mb-3">
                ...
                {filteredCustom.map((item) => {
                  ...
                  return <button type="button" key={item} onClick={() => toggle(item)} className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-left text-sm transition ${selected ? "bg-[#d4ad55]/10 font-bold text-[#8b6926]" : "text-slate-700 hover:bg-slate-50"}`}><span>{item}</span>{selected && <Check className="size-4" />}</button>;
                })}
              </div>
            )}
            {filtered.map((group) => (
