...
export function FinancialAccountsManager(){
  ...
  async function createAccount(ev:FormEvent){
    ...
    if(opening!==0) await supabase.from("financial_account_entries").insert({user_id:user.id,account_id:data.id,entry_type:"OPENING",amount:opening,description:"Saldo inicial",occurred_on:new Date().toISOString().slice(0,10)});
